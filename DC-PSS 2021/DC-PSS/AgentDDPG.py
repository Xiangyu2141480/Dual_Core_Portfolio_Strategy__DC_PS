...
class AgentDDPG(AgentBase):
    ...
    def init(self, net_dim=256, state_dim=8, action_dim=2, reward_scale=1.0, gamma=0.99,
             learning_rate=1e-4, if_per_or_gae=False, env_num=1, gpu_id=0,if_use_per=False):
        """
    ...
    def loss_td(self, q_values, next_q_values, rewards, dones, gamma):
        """
        Calculate the TD error for Q-learning.

        :param q_values: Q-values for the current state and action (shape: [batch_size, num_actions]).
        :param next_q_values: Q-values for the next state (shape: [batch_size, num_actions]).
        :param rewards: Immediate rewards (shape: [batch_size]).
        :param dones: Binary flags indicating episode termination (shape: [batch_size]).
        :param gamma: Discount factor.

        :return: TD error (shape: [batch_size]).
        """
