...
class ActorSAC(nn.Module):
    ...
    def __init__(self, mid_dim, state_dim, action_dim, if_use_dn=False):
        ...
        if if_use_dn:
            ...
            nn_middle = nn.Sequential(
                nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                # nn.Linear(mid_dim, mid_dim), nn.ReLU()  # 增加一层
            )
            inp_dim = mid_dim
            out_dim = mid_dim

        self.net_state = nn.Sequential(
            nn.Linear(state_dim, inp_dim), nn.ReLU(),
            nn_middle
        )
        self.net_a_avg = nn.Sequential(
            nn.Linear(out_dim, mid_dim), nn.Hardswish(),
            # nn.Linear(mid_dim, mid_dim), nn.Hardswish(),  # 增加一层
            nn.Linear(mid_dim, action_dim)
        )
        self.net_a_std = nn.Sequential(
            nn.Linear(23200, mid_dim), nn.Hardswish(),
            # nn.Linear(mid_dim, mid_dim), nn.Hardswish(),  # 增加一层
        ...
        self.conv_CAPM = nn.Sequential(
            nn.Conv2d(1, 8, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0)),
            nn.PReLU(),
            nn.Conv2d(8, 16, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0)),
            nn.PReLU(),
            nn.Conv2d(16, 32, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0)),
            nn.PReLU(),
            # nn.Conv2d(32, 64, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0)),  # 增加一层
            # nn.PReLU(),
            nn.MaxPool2d(kernel_size=(3, 1), stride=(1, 1), padding=(1, 0)),
            nn.Flatten()
        )
        self.fc3 = nn.Sequential(
            nn.Linear(23200, mid_dim), nn.ReLU(),
            # nn.Linear(mid_dim, mid_dim), nn.ReLU(),  # 增加一层
            nn.Linear(mid_dim, 29), nn.Softmax(dim=-1)
        )
    ...
    def forward(self, state):
        ...
        if state.dim() == 3:
            state = state.unsqueeze(3)  # 如果是三维的，添加一个批次维度
        ...
        action = self.fc3(x)  # Apply tanh activation for bounded actions
    ...
    def get_action(self, state):
        ...
        state = state.permute(0, 3, 1, 2)
        x = self.conv_CAPM(state)
        a_avg = self.fc3(x)  # Apply tanh activation for bounded actions
        a_std = self.net_a_std(x).clamp(-20, 0).exp()
        action = self.Softmax(torch.normal(a_avg, a_std))
        return action # re-parameterize
    ...
    def get_action_logprob(self, state):
        ...
        x = self.conv_CAPM(state)
        a_avg = self.fc3(x)  # Apply tanh activation for bounded actions
        a_std_log = self.net_a_std(x).clamp(-20, -10)
        a_std = a_std_log.exp()
