...
import torch
import numpy as np
from Build_env import build_env, build_eval_env
...
def train_and_evaluate(args, learner_id=0):  # 2021.11.11
    ...
    if env.env_num == 1:
        ...
        # assert agent.states[0].shape in {(env.state_dim, env.window_size - 6), env.state_dim}
        assert agent.states[0].shape in {(env.window_size, env.stock_dim, 1)}
    ...
    if args.if_off_policy:
        ...
        def update_buffer(_traj_list):
            # ten_state, ten_other, _, _ = _traj_list[0]
            ...
            _steps, _r_exp = get_step_r_exp(ten_other=ten_other)
    ...
    while if_train:
        ...
        with torch.no_grad():
            if_reach_goal, if_save = evaluator.evaluate_and_save(agent.agents, steps, r_exp, logging_tuple)
...
def get_step_r_exp(ten_other):
    return len(ten_other), ten_other.mean().item()
