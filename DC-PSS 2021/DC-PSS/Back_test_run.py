...
# eposide_return_DDPG_BL, return_mean_DDPG_BL, variance_DDPG_BL, sharpe_ratio_DDPG_BL = mean_and_variance_calculation(episode_total_assets, "DDPG-FINRL")
eposide_return_MADDPG_FinRL, return_mean_MADDPG_FinRL, variance_MADDPG_FinRL, sharpe_ratio_MADDPG_FinRL, output_data_list_FinRL = mean_and_variance_calculation(episode_total_assets, "DC-PSS-FinRL")
...
column= ["AR", "DR", "Std", "SR", "PD", "LStd", "STR"]
# print("index", index.shape())
index  = ["DC-PSS"]
import pandas as pd
Empirical_result = pd.DataFrame(columns=column, data = output_data_list_including_all_strategies[:, :, 0], index = index)
Empirical_result.to_csv("Empirical_result_in_the_back_test.csv", sep="," ,encoding='utf-8')
