    ...
    def evaluate_and_save(self, act, steps, r_exp, log_tuple) -> (bool, bool):  # 2021-09-09
        ...
        if time.time() - self.eval_time < self.eval_gap:
            ...
            if if_save:  # save checkpoint with highest episode return
                ...
                act_name = 'actor_ddpg' if self.if_overwrite else f'actor.{self.r_max:08.2f}'
                act_path = f"{self.cwd}/{act_name}.pth"
                torch.save(actor_ddpg.act.state_dict(), act_path)  # save policy network in *.pth
                act_name = 'actor_sac' if self.if_overwrite else f'actor.{self.r_max:08.2f}'
                act_path = f"{self.cwd}/{act_name}.pth"
                torch.save(actor_sac.act.state_dict(), act_path)  # save policy network in *.pth
...
def get_episode_return_and_step(env, act) -> (float, int):  # [ElegantRL.2021.10.13]
    ...
    act1 = act[0].act
    act2 = act[1].act
    device1_id = next(act1.parameters()).get_device()  # net.parameters() is a python generator.
    device = torch.device('cpu' if device1_id == -1 else f'cuda:{device1_id}')
    device2_id = next(act2.parameters()).get_device()  # net.parameters() is a python generator.
    device = torch.device('cpu' if device2_id == -1 else f'cuda:{device2_id}')
    ...
    if if_discrete:
        ...
        def get_action(_state):
            _state = torch.as_tensor(_state, dtype=torch.float32, device=device)
            _action = 0.5 * act1(_state.unsqueeze(0))[0] + 0.5 * act2(_state.unsqueeze(0))[0]
            return _action.detach().cpu().numpy()
