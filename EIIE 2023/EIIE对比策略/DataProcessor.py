import numpy as np
import pandas as pd
from YahooFianceProcessor import YahooFinanceProcessor
...
class DataProcessor:
    ...
    def clean_data(self, df,time_interval) -> pd.DataFrame:
        df = self.processor.clean_data(df, time_interval)
    ...
    def add_vix(self, df, time_interval) -> pd.DataFrame:
        df = self.processor.add_vix(df, time_interval)

        return df

    def df_to_array(self, df, if_vix) -> np.array:
        price_array, tech_array, turbulence_array, OCLC_array, close_in_OCLC_array = self.processor.df_to_array(
        ...
        return price_array, tech_array, turbulence_array, OCLC_array, close_in_OCLC_array