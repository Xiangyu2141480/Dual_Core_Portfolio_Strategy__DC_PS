...
class ReplayBuffer:
    def __init__(self, max_len, state_dim, action_dim, if_use_per, gpu_id=0, state_type=torch.float32):
        ...
        buf_state_shape = (max_len, 200, 29, 1) if isinstance(state_dim, int) else (max_len, *state_dim)
    ...
    def extend_buffer(self, state, other):
        ...
        #     raise ValueError("other 必须是一个列表或 NumPy 数组")
...
class BinarySearchTree:
    ...
    def get_indices_is_weights(self, batch_size, beg, end):
        ...
        eights = np.power(prob_ary, -self.per_beta)  # important sampling weights
