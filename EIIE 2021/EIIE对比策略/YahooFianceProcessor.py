...
class YahooFinanceProcessor:
    ...
    def add_vix(self, data, time_interval):
        """
        ...
        df_vix = self.clean_data(df_vix, time_interval)
    ...
    def df_to_array(self, df, tech_indicator_list, if_vix):
        ...
        for tic in unique_ticker:
            if if_first_time:
                ...
                OCLC_array = df[df.tic == tic][OHLC_LIST].values
                close_in_OCLC_array = df[df.tic == tic][["close"]].values
                ...
                OCLC_array = np.hstack(
                    [OCLC_array, df[df.tic == tic][OHLC_LIST].values]
                )
                close_in_OCLC_array = np.hstack(
                    [close_in_OCLC_array, df[df.tic == tic][["close"]].values]
                )
    ...
    def get_trading_days(self, start, end):
        ...
        df = nyse.sessions_in_range(
            pd.Timestamp(start), pd.Timestamp(end) #pd.Timestamp(start, tz=pytz.UTC), pd.Timestamp(end, tz=pytz.UTC)
        )
        trading_days = []
        for day in df:
