...
class AgentBase:  # [ElegantRL.2021.11.11]
    def __init__(self, net_dim=256, state_dim=8, action_dim=2, reward_scale=1.0, gamma=0.99,
        ...
        self.act = self.act_target = self.if_use_act_target = self.act_optim = self.act_predict_loss_optim = self.ClassAct = None
    ...
    def init(self, net_dim=256, state_dim=8, action_dim=2, reward_scale=1.0, gamma=0.99,
        ...
        self.cri_optim = torch.optim.Adam(self.cri.parameters(), learning_rate * 0.02)
        self.act_optim = torch.optim.Adam(self.act.parameters(), learning_rate, weight_decay = 1e-8) if self.ClassAct else self.cri
        self.act_predict_loss_optim = torch.optim.Adam(self.act.parameters(), learning_rate * 10)
    ...
    def explore_one_env_for_deep_learning(self, env, target_step: int) -> list:
        traj = list()
        ...
        for i in range(env.max_step):
            ...
            ten_state_EIIE = torch.as_tensor(state_EIIE, dtype=torch.float32)
            ten_future_return_based_on_closing = torch.as_tensor(future_return_based_on_closing, dtype=torch.float32)
            ten_action = self.select_actions(ten_state_EIIE.unsqueeze(0))[0]
            action = ten_action.numpy()
            next_s, next_state_EIIE, next_future_return_based_on_closing, reward, done, _ = env.step(action)
            ten_other = torch.empty(2 + self.action_dim)
            ten_other[0] = reward
            ten_other[1] = done
            ten_other[2:] = ten_action
            traj.append((ten_state, ten_action, ten_state_EIIE, ten_future_return_based_on_closing, ten_other))
            if done:
                break
            else:
                state = next_s
                state_EIIE = next_state_EIIE
                future_return_based_on_closing = next_future_return_based_on_closing

        traj_state = torch.stack([item[0] for item in traj])
        traj_action = torch.stack([item[1] for item in traj])
        traj_state_EIIE = torch.stack([item[2] for item in traj])
        traj_future_return_based_on_closing = torch.stack([item[3] for item in traj])
        traj_other = torch.stack([item[4] for item in traj])
        traj_list = [(traj_state, traj_action, traj_state_EIIE, traj_future_return_based_on_closing, traj_other), ]
        return self.convert_trajectory(traj_list)

    def explore_one_env_for_updating_portfolio_weights_vector(self, env, target_step: int) -> list:
        traj = list()
        state, state_EIIE, future_return_based_on_closing = env.reset()
        for i in range(env.max_step):
            ten_state = torch.as_tensor(state, dtype=torch.float32)
            ...
            ten_other[1] = done
            ten_other[2:] = ten_action
            traj.append((ten_action, ten_other))
            if done:
                break
            else:
                state = next_s
        ...
        # traj_state = torch.stack([item[0] for item in traj])
        # traj_state_EIIE = torch.stack([item[1] for item in traj])
        # traj_future_return_based_on_closing = torch.stack([item[2] for item in traj])
        traj_action = torch.stack([item[0] for item in traj])
        traj_other = torch.stack([item[1] for item in traj])
        traj_list = [(traj_action, traj_other), ]
        return self.convert_trajectory_for_portfolio_weights(traj_list)
    ...
    def optim_update(self, optimizer, objective):  # [ElegantRL 2021.11.11]
        ...
        """
        # prob_fake = objective(fake)  # 判别网络对生成数据的判断
        optimizer.zero_grad()
    ...
    def save_or_load_agent(self, cwd: str, if_save: bool):
        ...
        def load_torch_file(model_or_optim, _path):
            ...
            ### model_or_optim.load_state_dict(state_dict, strict = False)
    ...
    def convert_trajectory(self, traj_list: list) -> list:  # off-policy
        ...
        """
        for ten_state, traj_action, traj_state_EIIE, traj_future_return_based_on_closing, ten_other in traj_list:
            ten_other[:, 0] = ten_other[:, 0] * self.reward_scale  # ten_reward
            # ten_other[:, 1] = (1.0 - ten_other[:, 1]) * self.gamma  # ten_mask = (1.0 - ary_done) * gamma
            ten_other[:, 1] = self.gamma
            # print("self.reward_scale:",self.reward_scale)
            # print("self.gamma:",self.gamma)
        return traj_list

    def convert_trajectory_for_portfolio_weights(self, traj_list: list) -> list:  # off-policy
        for traj_action, ten_other in traj_list:
            ten_other[:, 0] = ten_other[:, 0] * self.reward_scale  # ten_reward
            # ten_other[:, 1] = (1.0 - ten_other[:, 1]) * self.gamma  # ten_mask = (1.0 - ary_done) * gamma
            ten_other[:, 1] = self.gamma
            # print("self.reward_scale:",self.reward_scale)
