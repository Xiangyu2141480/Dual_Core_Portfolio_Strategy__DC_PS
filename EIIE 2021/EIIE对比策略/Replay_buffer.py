...
class ReplayBuffer:
    def __init__(self, max_len, state_dim, action_dim, if_use_per, gpu_id=0, state_type=torch.float32):
        """Experience Replay final_modified_return_vector
        ...
        buf_state_shape = (max_len, 51, action_dim - 1, 1) if isinstance(state_dim, int) else (max_len, *state_dim)
        self.buf_state = torch.empty(buf_state_shape, dtype=state_type, device=self.device)

        buf_state_EIIE_shape = (self.max_len, 51, self.action_dim - 1, 3) if isinstance(self.state_dim, int) else (self.max_len, *self.state_dim)
        self.buf_state_EIIE = torch.empty(buf_state_EIIE_shape, dtype=state_type, device=self.device)

        buf_future_return_based_on_closing_shape = (self.max_len, 1, self.action_dim - 1, 1) if isinstance(self.state_dim, int) else (self.max_len, *self.state_dim)
        self.buf_future_return_based_on_closing = torch.empty(buf_future_return_based_on_closing_shape, dtype=state_type, device=self.device)
    ...
    def construct_new_buffer_of_action_and_other(self,):
        self.next_id = 0
        ...
        action_dim = self.action_dim
        self.buf_action = torch.empty((self.max_len, action_dim), dtype=torch.float32, device=self.device)

        # buf_state_shape = (self.max_len, 51, self.action_dim - 1, 1) if isinstance(self.state_dim, int) else (self.max_len, *self.state_dim)
        # self.buf_state = torch.empty(buf_state_shape, dtype=state_type, device=self.device)
        #
        # buf_state_EIIE_shape = (self.max_len, 51, self.action_dim - 1, 3) if isinstance(self.state_dim, int) else (self.max_len, *self.state_dim)
        # self.buf_state_EIIE = torch.empty(buf_state_EIIE_shape, dtype=state_type, device=self.device)
    ...
    def extend_buffer(self, state, action, state_EIIE, future_return_based_on_closing, other):
        size = len(other)
        self.max_len = state_EIIE.size(dim = 0)  # 705
        ...
        if next_idx > self.max_len:
            ...
            self.buf_action[self.next_id:self.max_len] = action[:self.max_len - self.next_id]
            self.buf_state_EIIE[self.next_id:self.max_len] = state_EIIE[:self.max_len - self.next_id]
            self.buf_future_return_based_on_closing[self.next_id:self.max_len] = future_return_based_on_closing[:self.max_len - self.next_id]
            ...
            self.buf_state_EIIE[0:next_idx] = state_EIIE[-next_idx:]
            self.buf_future_return_based_on_closing[0:next_idx] = future_return_based_on_closing[-next_idx:]
            self.buf_other[0:next_idx] = other[-next_idx:]
        else:
            self.buf_state[self.next_id:next_idx] = state
            self.buf_action[self.next_id:next_idx] = action
            self.buf_state_EIIE[self.next_id:next_idx] = state_EIIE
            self.buf_future_return_based_on_closing[self.next_id:next_idx] = future_return_based_on_closing
            self.buf_other[self.next_id:next_idx] = other
        self.next_id = next_idx

    def extend_buffer_of_actions_others(self, action, other):
        size = len(other)
        self.max_len = action.size(dim = 0)  # 705
        next_idx = self.next_id + size
        # print("self.next_id:",self.next_id)
        # print("next_idx:",next_idx)
    ...
    def extend_buffer_portfolio_weights(self, other):
        self.next_id = 0
        size = len(other)
        next_idx = self.next_id + size
        self.buf_other[self.next_id:next_idx] = other
    ...
    def __sample(self, start, end, bias = 5e-5):
        """
        ...
        while ran > end - start:
            ran = np.random.geometric(bias)
        result = end - ran
        indices = np.arange(result, result + 32)
        # print("result", result)
        return indices
    ...
    def sample_batch(self, batch_size) -> tuple:
        ...
        if self.per_tree:
            ...
            """
            random_numbers = np.random.geometric(p)
            """
            indices = self.__sample(0, self.now_len - 1 - batch_size, bias=5e-3)
            # print("===============================================")
            r_m_a = self.buf_other[indices]
            return (r_m_a[:, 0:1],  # reward
                    ...
                    self.buf_action[indices - 1],
                    self.buf_action[indices],
                    self.buf_state_EIIE[indices],
                    self.buf_future_return_based_on_closing[indices],
                    self.buf_state[indices + 1],
                    indices)

    def update_action_buffer(self, action_update, indices):
        self.buf_action[indices] = action_update
    ...
    def save_or_load_history(self, cwd, if_save, buffer_id=0):  # [ElegantRL.2021.11.11]
        ...
        if if_save:
            ...
            buf_state = np.empty((self.now_len, state_dim, 29, 6), dtype=buf_state_data_type)
